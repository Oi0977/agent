...
if __name__ == "__main__":
    ...
    ans1, hist, st1 = agent_run("Wireshark 怎么解密 HTTPS 流量?")
    print(f"\n第1轮答案(前80字): {ans1[:80]}")
    print(f"第1轮 token 统计: {st1}")
    time.sleep(3)  # 免费模型 429 限流,轮间稍隔
    ans2, hist, st2 = agent_run("你说的第二步里的 SSL 协议设置,具体在哪个菜单打开?", history=hist)
    print(f"\n第2轮追问答案(前80字): {ans2[:80]}")
    print(f"第2轮 token 统计: {st2}(对比第1轮,看历史重发的成本)")
